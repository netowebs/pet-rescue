...
import { ItensStockUpdateModel } from '../models/ItensStockUpdateModel';

export const itensStockUpdateList = async (req: Request, res: Response) => {
    try {
        let list = await ItensStockUpdateModel.findAll();
        res.send(list)
    } catch (error) {
        console.log(error)
    }
