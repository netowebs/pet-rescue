...
export const usersList = async (req: Request, res: Response) => {
    try {
        let list = await UsersModel.findAll();
        res.send(list)
    } catch (error) {
        console.log(error)
    }
...
export const usersDetail = async (req: Request, res: Response) => {
    try {
        let detail = await UsersModel.findOne({ where: { id: req.params.idUsers }})
        .then((data) => {
            if(data?.id){
...
export const usersDelete = async (req: Request, res: Response) => {
    try {
        const delSect = await UsersModel.destroy({
